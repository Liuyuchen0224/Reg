...
def test():
    ...
    for file in test_file_lst:
        ...
        if '6' in file:
            save_image(pred_img, f_img, "6_warped.nii.gz")
            save_image(pred_flow.permute(0, 2, 3, 4, 1)[np.newaxis, ...], f_img, "6_flow.nii.gz")
            save_image(pred_label, f_img, "6_label.nii.gz")
